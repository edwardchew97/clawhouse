...
export ACCOUNT_ID=your-operation-account.testnet
export CLAWHOUSE_OPERATION_KEY_FILE=~/.clawhouse/agents/<agent_id>/operation-key.json
```

For ClawHouse onboarding, use the operation key file generated by
`tools/near-wallet`. The file is local plaintext `0600` storage for Phase A; do
not paste its private key into chat, Workbench, logs, or repo files.

If `CLAWHOUSE_OPERATION_KEY_FILE` is not set, the scripts try
`NEAR_PRIVATE_KEY`, then:
...
```sh
STORAGE_DEPOSIT=0.05 bun run create terminal_chad "Terminal Chad" ipfs://terminal-chad
```
...
```sh
STORAGE_DEPOSIT=0.05 bun run buy terminal_chad 1 0.06
```
