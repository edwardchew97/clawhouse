...
- The repo root at `/Users/Edward/Documents/clawhouse` is the protected local integration checkout. It must stay clean during active work; if `git status --short` shows changes there before a task starts, stop before adding more work and either report the dirt or resolve it with explicit approval.
...
- The local main checkout may only update by syncing `dev` from GitHub, preferably by fast-forward pull. Do not check out feature branches, merge/rebase feature branches, cherry-pick, commit, or push feature work from the local main checkout unless JY explicitly orders that repo-hygiene exception.
